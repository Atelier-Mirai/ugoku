...
$('#chart02').on('inview', function (event, isInView) { //画面上に入ったらグラフを描画
  if (isInView) {
    let ctx02 = document.getElementById("chart02"); //グラフを描画したい場所のid
    ctx02.height = 300; //グラフの高さ
    let chart02 = new Chart(ctx02, {
      type: 'bar', //グラフのタイプ
      data: { //グラフのデータ
...
function RandomAnimeControl() {
  let elemPos = $(element[1]).offset().top - 50;
...
particlesJS("particles-js", {
  "particles": {
    "number": {
      "value": 38,
      /*この数値を変更すると幾何学模様の数が増減できる*/ "density": {
        "enable": true,
        "value_area": 800
      }
    },
    ...
    "move": {
      "enable": true,
      "speed": 6,
      /*この数値を小さくするとゆっくりな動きになる*/ "direction": "none",
      /*方向指定なし*/ "random": false,
      /*動きはランダムにしない*/ "straight": false,
      /*動きをとどめない*/ "out_mode": "out",
      /*画面の外に出るように描写*/ "bounce": false,
      /*跳ね返りなし*/ "attract": {
        "enable": false,
        "rotateX": 600,
